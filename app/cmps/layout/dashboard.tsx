...
export const Dashboard = () => {
  ...
  let isOwned = false
  if (selectedCell) {
    selectedPrice = calculateCellPrice(selectedCell.x, selectedCell.y)
  ...
  const handleBuyClick = () => {
    if (selectedCell) {
      openPaymentModal(selectedCell.index)
    }
  }
  ...
  return (
    <div className="fixed right-4 top-24 bottom-4 w-80 rounded-2xl bg-black/60 backdrop-blur-xl border border-white/10 p-6 flex flex-col gap-8 z-40 overflow-y-auto font-sans text-white shadow-2xl">
      <div>
        <h2 className="text-sm uppercase tracking-widest text-white/50 mb-1">
          Portfolio Balance
        </h2>
      ...
      <div className="bg-white/5 rounded-xl p-4 border border-white/10">
        ...
        {!selectedCell ? (
          <p className="text-sm text-white/40 italic">
            Select a cell in the grid to view details.
          </p>
        ) : (
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <span className="text-xs text-white/60 font-mono">
                ID: {selectedCell.index}
              </span>
            ...
            {isOwned ? (
              ...
              <Button
                ...
                className="w-full mt-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold"
              >
                Purchase Cell
              </Button>
            )}
        ...
        {Object.keys(ownedCells).length === 0 ? (
          ...
          <div className="flex flex-col gap-2 overflow-y-auto pr-2">
            {Object.entries(ownedCells).map(([index, data]) => (
              <div
                key={index}
                className="flex items-center justify-between bg-white/5 rounded-lg p-3 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer"
                onClick={() => {
                  if (voroforce?.controls) {
                    voroforce.controls.navigateToCellById(parseInt(index))
                  }
                }}
              >
